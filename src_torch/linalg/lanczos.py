...
def extend_lanczos_basis(
    ...
    """Extend a CG-derived orthonormal basis by Lanczos iterations.

    This version starts from an existing basis ``q_mat`` with shape
    ``[batch, n, J]``. In the prototype, this basis is usually obtained from
    QR-orthogonalized CG search directions. The function then continues the
    Lanczos process until either ``max_iter`` vectors have been reached,
    numerical breakdown occurs, or reorthogonalization fails.

    Unlike the recovered-matrix variant, this function does not propagate a
    projected matrix during the extension. Instead, after the final basis has
    been constructed, the projected matrix is computed directly as

        T = Q.T @ A @ Q,

    where multiplication by ``A`` is represented by ``matmul_closure``. This is
    more expensive than updating the recurrence coefficients alone, but it is
    numerically more robust for the CG-derived basis.

    Args:
        matmul_closure: Callable implementing multiplication by the system
            matrix.
        max_iter: Maximum final basis size.
        dtype: Floating point dtype used for the basis.
    ...
    Returns:
        ...
        ``[batch, n, J_final]`` and ``t_final = q_final.T @ A @ q_final`` has
        shape ``[batch, J_final, J_final]``.
    """
    ...
    # If no extension is needed, truncate and compute the projected matrix directly
    ...
    # Recover the last diagonal coefficient from q^T A q
    ...
    # If available, remove the previous basis-vector component
    if current_iter > 1:
    ...
    for k in range(current_iter, target_iter):
        ...
        # Normalize the residual to obtain the next basis vector
        q_prev = q
        q = r_vec.div(beta)
        ...
        # Compute A q_k and the diagonal coefficient alpha_k
        r_vec = matmul_closure(q)
        ...
        # Full reorthogonalization against all stored basis vectors
    ...
    # Compute the projected matrix directly for the final basis
    t_final = torch.matmul(q_final.transpose(-1,-2), matmul_closure(q_final))
    return q_final, 0.5 * (t_final + t_final.transpose(-1, -2))